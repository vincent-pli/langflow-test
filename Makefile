...
build_and_run:
	...
	rm -rf dist
	# make build && poetry run pip install dist/*.tar.gz && poetry run langflow --components-path /Users/lipeng/workspaces/github.com/logspace-ai/langflow/new-components --log-level debug 
	make build && poetry run pip install dist/*.tar.gz && poetry run langflow --log-level debug 
